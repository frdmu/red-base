...
RC IX_IndexHandle::insert_internal(int nodeNum, int *splitNode, void *pData, const RID &rid) {	
	...
	*splitNode = kNullNode;
	...
	int index = n - 1;
	for (int i = 0; i < n-1; ++i) {
		Entry *entry = (Entry*)get_entry(entries, i);
	...
	if (lowerSplitNode != kNullNode) {
		printf("insert_internal: lower split\n");
		PF_PageHandle s_ph;
		...
		if (n != this->b) {
			...
			if (index != n - 1) {
				...
				memmove(src + entrySize, src, (char*)__get_entry(entries, n) - src);
			}
			memcpy(((Entry*)__get_entry(entries, index))->key, s_key, attrLength);
...
RC IX_IndexHandle::insert_leaf(int nodeNum, int *splitNode, void *pData, const RID &rid) {
	...
	*splitNode = kNullNode;
	...
	if (n != this->b - 1) {
		...
		TRY(s_ph.GetData(CVOID(s_header)));
		short &s_n = s->header->childrenNum;
...
RC IX_IndexHandle::InsertEntry(void *pData, const RID &rid) {
	...
	if (splitNode != kNullNode) {
		...
		memcpy(p_entry_0->key, ((Entry*)__get_entry(s_header->entries, 0))->key, attrLength);
