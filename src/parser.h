...
struct Condition {
	...
	int 	bRhsIsAttr; /* TRUE if the rhs is an attribute, in which case rhsAttr below is valid;*/
...
void RBparser(PF_Manager &pfm, SM_Manager &smm, QL_Manager &qlm);

// Error printing function; calls component-specific functions
void PrintError(RC rc);

// bQueryPlans is allocate by parser.y. When bQueryPlans is 1 then the
// query plan chosen for the SFW query will be displayed. When
// bQueryPlan is 0 then no query plan is shown.
