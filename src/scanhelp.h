 ...
 * Otherwise, it returns the token value corresponding to a string. If s is longer than the maximum token
 * length (MAXSTRINGLEN) then it returns NOTOKEN, so that the parser will flag an error (this is a stupid kludge).
 */
